...
Опкод **32** `{contactIds}` отвечает `{contacts: [...]}`, где каждый контакт
несёт `names: [{name, firstName, lastName}]`, `baseRawUrl` (аватар на
`i.oneme.ru`), `description` (описание профиля), `options` — **массив** строк,
в котором `OFFICIAL` означает верификацию, а также `photoId`, `accountStatus`
